...
describe("Auth Service - wxLogin", () => {
  ...
  describe("Simple flow - no unionid", () => {
    it("creates new user when openid does not exist", async () => {
      ...
      const result = await wxLogin(prismaMock, code);
  ...
  describe("Complex flow - with unionid", () => {
    it("finds user by unionid and updates openid if different", async () => {
      ...
      const userWithOldOpenid = {
        id: 3,
        openid: "old_openid",
        unionid: "existing_unionid",
      };
      const userWithNewOpenid = {
        id: 3,
        openid: "new_openid",
        unionid: "existing_unionid",
      };
    ...
    it("finds user by openid and adds unionid if missing", async () => {
      ...
      const userWithoutUnionid = {
        id: 5,
        openid: "existing_openid",
        unionid: null,
      };
      const userWithUnionid = {
        id: 5,
        openid: "existing_openid",
        unionid: "new_unionid",
      };
    ...
    it("creates new user with both openid and unionid", async () => {
      ...
      const wxSession = {
        openid: "brand_new_openid",
        unionid: "brand_new_unionid",
      };
      ...
      // Mock the flow: findUnique by unionid returns null, findUnique by openid returns null, create returns newUser
      prismaMock.user.findUnique
        .mockResolvedValueOnce(null) // First call for unionid
        .mockResolvedValueOnce(null); // Second call for openid
      prismaMock.user.create.mockResolvedValue(newUser as any);
  ...
  describe("Error handling", () => {
    it("throws error when WeChat API returns error code", async () => {
      ...
      await expect(wxLogin(prismaMock, code)).rejects.toThrow(
        "WeChat API Error: invalid code",
      );
    });
    ...
    it("throws error when WeChat API request fails", async () => {
      ...
      await expect(wxLogin(prismaMock, code)).rejects.toThrow("Network error");
    });
    ...
    it("throws error when database transaction fails", async () => {
      ...
      await expect(wxLogin(prismaMock, code)).rejects.toThrow("Database connection failed");
    });
  });
