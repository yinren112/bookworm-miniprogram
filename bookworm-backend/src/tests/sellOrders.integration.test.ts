...
describe("Sell Orders API", () => {
  ...
  it("allows staff to create and complete a sell order", async () => {
    const { userId: sellerId } = await createTestUser("USER");
    ...
    // cost和selling_price在数据库中以Decimal存储（yuan为单位）
    // 3.2kg * 2.50 yuan/kg = 8.00 yuan
    // 使用toBeCloseTo避免浮点数精度问题
    expect(Number(inventoryItem.cost)).toBeCloseTo(8.0);
    expect(Number(inventoryItem.selling_price)).toBe(0);
  });

  it("rejects non-staff callers", async () => {
    const { userId: sellerId, token } = await createTestUser("USER");
